...
def data_window():
    ...
    def refresh(window):
        ...
        tkinter.Label(data_manupulation_window, text='Guests').grid(row=0, column=0, columnspan=2)
        tkinter.Label(data_manupulation_window, text='Prizes').grid(row=0, column=3, columnspan=2)
...
def get_winner():
    ...
    if guest_list and prize_list:
        ...
        try:
            ...
            tkinter.Label(root, text=('The winner is ' + str(winner[0][1]) + '. He won the ' + str(winner[1][1]))).grid(
                row=4,
                column=5)
